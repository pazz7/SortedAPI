...
namespace Sorted.Test
{
    public class RainfallControllerTest
    {
        ...
        {
            // Act
            var okResult = await _controller.GetRainfall("WithDataStationId", 15) as OkObjectResult;

            // Assert
            var response = Assert.IsType<RainfallReadingResponse>(okResult?.Value);
            Assert.Equal(15, response.Readings.Length);
        }

        [Fact]
        public async void GetByStationId_WhenCalled_ResturnsStationReadingsWithDefaultCount()
        {
            // Act
            var okResult = await _controller.GetRainfall("WithDataStationId") as OkObjectResult;
